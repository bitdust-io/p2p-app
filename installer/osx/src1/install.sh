...
PYTHON_DIST_DIR="${CURRENT_DIR}/python"
...
if [[ ! -e $SOURCE_UI_DIR ]]; then
    ...
    $PYTHON_BIN -c "import pygit2; pygit2.clone_repository('https://github.com/bitdust-io/p2p-app.git', '$SOURCE_UI_DIR')"
else
    echo ''
    echo "##### Updating BitDust UI source files from Git repository"
    $PYTHON_BIN -c "import pygit2; repo=pygit2.init_repository('$SOURCE_UI_DIR'); repo.remotes[0].fetch(); top=repo.lookup_reference('refs/remotes/origin/master').target; repo.reset(top, pygit2.GIT_RESET_HARD);"
fi
